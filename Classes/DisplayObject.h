...
namespace masi {
	...
	public:
		...
		void addChildAt(DisplayObject * displayObject, unsigned int index);
		...
		DisplayObject * removeChildAt(unsigned int index);
		...
		void setChildIndex(DisplayObject * child, unsigned int newIndex);
		...
		// does nothing.
		void setChildIndexAt(unsigned int oldIndex, unsigned int newIndex);
		...
		void moveChildAtToFront(unsigned int childIndex);
		...
		//if <code>childIndex</code> is smaller than <code>numChildren</code>, otherwise it has no effect.
		void moveChildAtToBack(unsigned int childIndex);
